<x-layout.layout-auth title="Daftar | Tasty Food">
  <div class="bg-white rounded-2xl shadow-xl w-full max-w-md px-8 py-10">
    <h2 class="text-2xl font-bold text-yellow-600 mb-2 text-center">Daftar</h2>
    ...
    <form action="{{ route('register') }}" method="POST" class="space-y-4">
      ...
      <button type="submit"
        class="w-full mt-2 bg-yellow-500 hover:bg-yellow-600 text-white font-bold py-2 px-4 rounded">
        DAFTAR
      </button>
    </form>
    ...
    <p class="mt-4 text-sm text-slate-600 text-center">
      Sudah punya akun?
      <a href="{{ route('login') }}" class="text-yellow-600 font-semibold">Masuk</a>
    </p>
  </div>
