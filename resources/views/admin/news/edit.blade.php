  ...
  <div class="p-4">
    ...
    <form action="{{ route('admin.news.update', $news->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
      ...
      <div>
        ...
        <input type="text" name="title" class="w-full border rounded px-3 py-2" value="{{ old('title', $news->title) }}">
      </div>
      ...
      <div>
        ...
        @if ($news->image)
          <img src="{{ asset('storage/' . $news->image) }}" class="w-32 h-20 object-cover rounded mb-2">
        @else
          <p>-</p>
        @endif
      ...
      <div>
        <button class="bg-amber-300 text-white font-bold px-4 py-2 rounded hover:bg-amber-400">Update</button>
      </div>
    </form>
  </div>
</x-layout.admin-layout>
