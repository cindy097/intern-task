<x-layout.admin-layout>
  <div class="p-4">
    <div class="bg-white rounded-lg shadow p-6 max-w-xl mx-auto">
      <h1 class="text-2xl font-bold mb-6">Edit Slideshow</h1>
      ...
      <form action="{{ route('admin.slideshows.update', $slideshow->id) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
        @csrf
        @method('PUT')
        ...
        <div>
          ...
          <img src="{{ asset('storage/' . $slideshow->image) }}" class="h-32 mb-2 rounded shadow-sm">
        </div>
